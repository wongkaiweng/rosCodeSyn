...
def make_joint_limit_dic_from_robObj(robot):
    dic={}
...
def make_vel_limit_dic_from_yaml_file(yaml_file):
    try:
        return make_vel_limit_dic_from_yaml_text(open(yaml_file,'r'))
    except:
        return None


def make_vel_limit_dic_from_yaml_text(yaml_text):
    if yaml_text:
